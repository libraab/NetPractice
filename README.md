...
Step 2)- In order for A1 & B1 to communicate with each other, they must have the same netmask (or subnet mask) and close IP addresses (same for C1 & D1).

  A1 will be assigned an IP address that is either immediately before or after B1's IP address.
  
  If the IP address that comes before doesn't work that means it is not assignable, try the IP address that comes after.

Note that an IP addr is not assignable if it's already taken by either the network or the broadcast, such as IP addresses ending with .0 or .255 .
...
Step 3)- Give Host C & B IP addresses close to Host A's IP.
...
The smaller the mask the larger range of IPs it will give you. But let's not be greedy; /29 is already more than we need.
...
step 5)- In the right section of Client B routes, put R2's IP.

step 6)- In the right section of Client A routes, put R1's IP.
...
Step 7)- Give to B1 an IP close to R1's IP and to A1 an IP close to R2's IP. (...255 and ...127 won't work cause they're already taken by the broadcast).

(Remember what we said about routers earlier (entrances & exits) it's like saying to the device "if you're looking for someone (left section) (default = anyone), go there (right section)". So in the right section of these blue/purple boxes you must put the IP of the closest router. If you want the safest way and you just wanna pass, then, ALWAYS put defaut in the left section and the IP of the closest router's entrance in the right section).
...
Seriously, if you're really curious and wanna know, it's just because I know that it will cover all the devices that have IP endings between .1 ans .245, the mask basically defines how you will section/sequence your network.   
...
Since for each network we only have two devices we can use a mask of /30 (that gives us a range of only 2 IP's which is enough).

By using a small range we avoid mistakes or intefering with other subnets.

If you use a mask that gives you a larger range we'll have to do some math and binary shit, if you already know how to do this be my guest and get the f out of this tuto for dummies.

So put a mask /30 for everyone.

step 4)- A1 must have the IP that comes right after R11 IP.

step 5)- For R21 put the IP that comes BEFORE R12's IP. (IPs that ends up with .255 are NEVER assignable cause they're already taken by the broadcast).
...
step 9)- When you have 2 routers linked to each other, in order to connect them we have to give to each of them the IP of the other (cause technically they're the closest router to each other); so put R12's IP in the right section of router R2 and put R21's IP in the right section of router R1.
...
Let's give them the same mask, which means 255.255.255.240. R22 & C1 masks could have the same mask aswell but /30 is enough, anyways both works; what's important is that the linked devices (by a wire) have the same mask.
...
(the mask for the devices on the left is 255.255.255.240 so by doing the substraction 255 - 240 = 15, I know that I have a range of 15 IP's, and when we will be assigning IP's in the next subnet we shall not interfere with this range, so that means ip's from 134.125.140.1 to 134.125.140.15 are taken by the subnet D1 & R23), then we start filling the ones on the right; I skip the address that ends with .16 cause it's the network IP of the subnet C1 & R22.
...
- R23 & D1 must have the same mask which means /18.
- In the right section of Gluon routes, we have an IP; that must be R23's IP.
- For C1 & R22 you can do as you want; but keep in mind that a small mask and close IPs will spare you some headache and frustration and existential questionning.
![level9-goal2done](https://user-images.githubusercontent.com/81954460/155990003-2bd89684-3929-455c-9db7-97b5537b6fa5.png)
Goal 3) --> Meson needs an internet connection, follow the steps on the screanshot.
...
Goal 2) --> R23 IP is already known (check H4 routes).
- For R22 and H31, I did the same substraction as in level 8.
- H41 mask is 255.255.255.192, calculating how many IP left from 192 till 255, 255 - 192 = 62 IPs.
- Starting from 170.135.223.129 --> 129 + 62 = 191, and I skip the following one cause it's the network IP of the next subnet.
